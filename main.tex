...
\begin{equation*}
        \sigma_y = \frac{1}{n}\sum_{t = 1}^{n} \left( y_t - \frac{1}{n}\sum_{i = 1}^{n}y_i \right)^2.
\end{equation*}
...
In turn this can contribute to detecting the clusters behaviour clusters.
...
The scales $s_1, s_2, \hdots, s_n$ in equation \eqref{eq:cwt_d} are chosen as fractional powers of two—as suggested in \cite{torrence}—e.g.,
\begin{align*}
        &s_j = \frac{2^{j\delta j}}{\omega_{\text{max}}}, \quad j = 0, 1, \hdots, J - 1, \\
