...
\begin{align}\label{eq:spectrum}
        f(w) &= \frac{1}{2 \pi} \sum_{h = -\infty}^{\infty} \gamma_h e^{-iwh}, \\
...
\begin{equation}\label{eq:sample_spectrum}
        \hat{f}(w) = \frac{1}{2\pi}\sum_{k = -(n - 1)}^{n-1}\hat{\gamma}_k e^{-iwk}.
\end{equation}
At the Fourier frequencies $w_k$ it is related to the periodogram through \cite{wei}
\begin{equation*}
        \hat{f}(w_k) = \frac{I(w_k)}{4 \pi}.
\end{equation*}
Although $\hat{f}(w_k)$ is asymptotically unbiased, meaning $\lim_{n \rightarrow \infty} E(\hat{f}(w)) = f(w)$, is lacks consistency in the variance as $n$ tends to infinity, i.e.,
\begin{equation*}
...
\begin{equation}\label{eq:smo_spec}
        \hat{f}_{\mathcal{W}}(w_k) = \sum_{j = -m}^{m} \mathcal{W}_n(w_j) \hat{f}(w_k - w_j),
\end{equation}
where $w_k = 2\pi k/n,\ k = 0,1, \hdots, (n-1)/2$ and $m$ is a function of $n$, typically $m \ll n$.
The value of $m$ decides how many points in the neighborhood of $w_k$ should be included in the smoothing.
Furthermore, the function $\mathcal{W}_n(w_j)$ is chosen to have the following properties,
\begin{align}
        &\sum_{j = -m}^{m}\mathcal{W}_n(w_j) = 1, \nonumber \\
        &\mathcal{W}_n(w_j) = \mathcal{W}_n(-w_j), \nonumber \\
        \label{eq:w_var}
        &\lim_{n \rightarrow \infty} \sum_{j = -m}^{m}\mathcal{W}_n^2(w_j) = 0.
\end{align}
We view the smoothed spectrum as a weighted average of the sample spectrum \eqref{eq:sample_spectrum} in a window around the target frequency $w_k$.
How the weights are distributed in the window is governed by $\mathcal{W}_n(w_j)$, giving it the name spectral window.
Because of the property in \eqref{eq:w_var} we have
\begin{align*}
        \text{Var}(\hat{f}_\mathcal{W}(w_k)) &\approx \sum_{j = -m}^{m}\mathcal{W}_n^2(w_j)(f(w_k))^2 \\
                                             &=(f(w_k))^2 \sum_{j = -m}^{m} \mathcal{W}_n^2(w_j) \overset{n \rightarrow \infty}{=} 0,
\end{align*}
assuming $f(w)$ is approximately constant in the window.
...
\begin{equation*}
        W_n(k) = \int_{-\pi}^{\pi}\mathcal{W}_n(w) e^{iwk}dw, \quad k = 0, \pm 1, \hdots, \pm M.
\end{equation*}
The lag window is a weighting function applied to the sample autocovariance 
\begin{equation*}
        \hat{f}_\mathcal{W} = \frac{1}{2\pi} \sum_{k = -M}^{M}W_n(k) \hat{\gamma}_k e^{-iwk},
\end{equation*}
...
\begin{align*}
        ...
        &W(t) = , \  t > 1.
\end{align*}
Figure \textcolor{red}{insert figure} shows the popular \textit{hanning} window given by 
